...
# データ読み込み / 前処理
# -----------------------
data = np.load(DATA_PATH).astype("float32")  # shape: (N, 200, 2)
...
vae.compile(optimizer=optimizer, loss=lambda y_true, y_pred: 0)

# -----------------------
# コールバック
# -----------------------
callbacks = [
    keras.callbacks.ModelCheckpoint(os.path.join(MODEL_DIR, "vae_best.weights.h5"), save_weights_only=True, save_best_only=True, monitor="loss"),
